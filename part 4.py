    ...
    while process: #process's value is assigned as True
        try:
            ...
            if choice == 'y' :
                ...
                print() #printing a blank tab

                file = open('marks.txt','w') #opening text file marks and writing in write-only mode instead of append mode,overwrite text file without appending
                for i in scores: #for all the content in list scores
                    file.write(i + '\n') #writing in marks text file keeping new tabs between each item
                file.close() #closing the previously opened file

                print() 
                try:
                        file_name = input('Enter the name of your file:') #enetering the file name
                        marks_file= open(file_name) #opening the entered file if it's in the same directory as the script
                        content = marks_file.read() #reading content in the file
                        print(content) #printing content of the file
                        marks_file.close()
                except FileNotFoundError: #if user entered a worng file which doesn't exist in the same directory as the python script
                     print('File not found') 
            
            
                process = False # loop breaks
    
            else: #if user enterd something else besides y and q
                error_message = print('Wrong input entered')
...
def students(): #function for students
    ...
    while process:
        try:
         PASS = int(input('Enter your number of credits at pass:'))
         if PASS not in range(0,130,20):
             out_of_range_PASS = print('Out of range') # not in range
             continue
         DEFER = int(input('Enter your number of credits at defer:')) #from 0 to 120,+20 steps (130 not included according to range)
         if DEFER not in range(0,130,20):
             out_of_range_DEFER = print('Out of range') # everything should be within 0-120 marks category
             continue
         FAIL = int(input('Enter your number of credits at fail:'))
         if FAIL not in range(0,130,20):
