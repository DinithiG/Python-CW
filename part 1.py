...
def students(): #function for students
    ...
    while process:
        try:
         ...
         if PASS not in range(0,130,20): # not in range 
             out_of_range_PASS = print('Out of range')
             continue
         DEFER = int(input('Enter your number of credits at defer:')) 
         if DEFER not in range(0,130,20): #from 0 to 120,+20 steps (130 not included according to range)
             out_of_range_DEFER = print('Out of range')
             continue
         FAIL = int(input('Enter your number of credits at fail:')) # everything should be within 0-120 marks category
         if FAIL not in range(0,130,20):
